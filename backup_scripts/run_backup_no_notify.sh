...
# Send notification via ntfy to the correct topic
curl -X POST "https://ntfy.sh/$NOTIFICATION_TOPIC" -H "Content-Type: application/json" -d "{"title": "$TITLE", "message": "$MESSAGE", "priority": $PRIORITY, "tags": ["$TAGS"]}"
