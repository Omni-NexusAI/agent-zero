...
run_backup() {
    ...
    echo "Debug: About to run tar command"
    echo "Debug: BACKUP_FILE=$BACKUP_FILE"
    echo "Debug: TEMP_PATHS=$TEMP_PATHS"
    echo "Debug: Contents of TEMP_PATHS:"
    cat "$TEMP_PATHS"
    ...
    echo "Debug: Tar command completed with exit code $?"
    echo "Debug: Backup file size:"
    ls -la "$BACKUP_FILE"
        else
...
' ', ')
    ...
    # Send notification via echo for now
    echo "🚨 Important System Changes Detected"
    echo "Detected important system changes that require backup inclusion:"
    echo "$COMPONENTS_PREVIEW"
    echo "These will be automatically included in the next backup run."
