...
def clear_plugin_cache(plugin_names: list[str] | None = None):
    ...
    # Notify frontend about cache clear (gracefully handle missing ws_manager)
    try:
        from python.helpers.websocket_manager import WebSocketManager
        # Use agentspine websocket manager if available
        pass  # TODO: Implement frontend notification when ws system is unified
    except ImportError:
        pass
    ...
    def load_plugins(root_path: str, is_custom: bool):
        for d in sorted(Path(root_path).iterdir(), key=lambda p: p.name):
            try:
                ...
                thumbnail_url = ""
                _thumb_exts = ("png", "jpg", "jpeg", "gif", "webp", "svg")
