...
try:
    from python.helpers import yaml_helper as yaml
except ImportError:
    try:
        from helpers import yaml_helper as yaml
    except ImportError:
        from helpers import yaml


# Update source configuration — user can switch between sources in settings
OFFICIAL_REPO_AUTHOR = "Omni-NexusAI"  # Default: Agentspine fork
OFFICIAL_REPO_NAME = "agent-zero"

# Alternative update sources
UPDATE_SOURCES = {
    "omni-nexusai": {"author": "Omni-NexusAI", "name": "agent-zero", "label": "Agentspine (Omni-NexusAI)"},
    "agent0ai": {"author": "agent0ai", "name": "agent-zero", "label": "Agent Zero (agent0ai)"},
}
DEFAULT_UPDATE_SOURCE = "omni-nexusai"
AGENTSPINE_PREVIEW_LATEST_TAG = "v0.9.9-pre"
...
def invalidate_caches() -> None:
    """Clear all remote branch/tag caches. Call when update source changes."""
    global _remote_branch_list_cache
    _remote_branch_tag_cache.clear()
    _remote_branch_head_cache.clear()
    _remote_branch_list_cache = None
...
def _get_active_source() -> dict:
    """Get the active update source configuration from settings."""
    try:
        try:
        ...
        source_key = current.get("self_update_source", DEFAULT_UPDATE_SOURCE)
        return UPDATE_SOURCES.get(source_key, UPDATE_SOURCES[DEFAULT_UPDATE_SOURCE])
    except Exception:
        return UPDATE_SOURCES[DEFAULT_UPDATE_SOURCE]


def _get_active_source_key() -> str:
    """Get the active update source key from settings."""
    try:
        try:
            from python.helpers import settings as settings_helper
        except ImportError:
        ...
        current = settings_helper.get_settings()
        source_key = current.get("self_update_source", DEFAULT_UPDATE_SOURCE)
        return source_key if source_key in UPDATE_SOURCES else DEFAULT_UPDATE_SOURCE
    except Exception:
        return DEFAULT_UPDATE_SOURCE


def _is_agentspine_source() -> bool:
    return _get_active_source_key() == "omni-nexusai"


def _is_agentspine_version(_version: str | None) -> bool:
    """Agentspine preview track uses Omni-NexusAI source; spoof applies when that source is active."""
    return _is_agentspine_source()
...
def get_current_major_main_latest_info(
    ...
    *,
    ...
    if _is_agentspine_source() and _is_agentspine_version(current_version):
        return {
            "branch": "main",
            "supported": True,
            "describe": AGENTSPINE_PREVIEW_LATEST_TAG,
            "short_tag": AGENTSPINE_PREVIEW_LATEST_TAG,
            "display_version": AGENTSPINE_PREVIEW_LATEST_TAG,
            "commit": "",
            "short_commit": "",
            "released_at": "",
        }
...
def get_available_tags(
    ...
    *,
    ...
    source = _get_active_source()
    result = git.get_remote_releases(source["author"], source["name"])
    if result.error:
        return [], result.error
...
def _is_cross_source_context(repo_dir: str | Path | None = None) -> bool:
    """Return True when the active update source differs from the installed repo origin."""
    try:
        repository = get_repo_dir(repo_dir)
        origin_url = _run_git(repository, "remote", "get-url", "origin").lower()
        active = _get_active_source()
        active_author = active["author"].lower()
        return active_author not in origin_url
    except Exception:
        return False
...
def get_selector_tag_options(
    ...
    *,
    ...
    cross_source = _is_cross_source_context(repo_dir=repository)
    ...
    for tag in tags:
        ...
        if tag_major is None:
            continue
        if cross_source or tag_major == current_major:
            same_major_tags.append({"value": tag, "label": tag})
        elif tag_major > current_major:
            higher_major_versions.add(tag_major)

    if not cross_source and branch_head_major is not None and branch_head_major > current_major:
    ...
    if supports_latest and normalized_branch == "main" and same_major_tags:
        release_label = (
            _format_latest_release_label(AGENTSPINE_PREVIEW_LATEST_TAG)
            if _is_agentspine_source()
            else _format_latest_release_label(same_major_tags[0]["value"])
        )
        ...
        supports_latest
        and (cross_source or branch_head_major == current_major)
...
def get_update_info(repo_dir: str | Path | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "update_sources": [
            ...
            for k, v in UPDATE_SOURCES.items()
        ],
        "active_update_source": _get_active_source_key(),
    }
