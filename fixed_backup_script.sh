...
# Fixed backup script with corrected exclude patterns
# Solves the exclude pattern issue that was causing 2KB backups
...
# Embed backup_info.txt (now that cpio is installed)
echo "/tmp/backup_info.txt" | cpio -o -H ustar -F "$BACKUP_FILE" 2>/dev/null
