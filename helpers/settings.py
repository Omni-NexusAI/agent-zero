...
def get_default_settings() -> Settings:
    ...
    return Settings(
        ...
        tts_device=get_default_value("tts_device", get_tts_defaults().get("tts_device", "auto")),
        tts_kokoro_voice=get_default_value("tts_kokoro_voice", get_tts_defaults().get("tts_kokoro_voice", "am_michael")),
        tts_kokoro_voice_secondary=get_default_value("tts_kokoro_voice_secondary", get_tts_defaults().get("tts_kokoro_voice_secondary", "")),
        tts_kokoro_voice_blend=get_default_value("tts_kokoro_voice_blend", get_tts_defaults().get("tts_kokoro_voice_blend", 50)),
        tts_kokoro_speed=get_default_value("tts_kokoro_speed", get_tts_defaults().get("tts_kokoro_speed", 1.1)),
        tts_kokoro_remote_url=get_default_value("tts_kokoro_remote_url", get_tts_defaults().get("tts_kokoro_remote_url", "")),
        tts_kokoro_remote_token=get_default_value("tts_kokoro_remote_token", get_tts_defaults().get("tts_kokoro_remote_token", "")),
        tts_kokoro_remote_timeout=get_default_value("tts_kokoro_remote_timeout", get_tts_defaults().get("tts_kokoro_remote_timeout", 20)),
        ...
        self_update_source=get_default_value("self_update_source", "omni-nexusai"),
...
def _apply_settings(previous: Settings | None):
    ...
    if _settings:
            ...
            try:
                ...
                if remote_selected:
                    ...
                    if policy_changed:
                        ...
                        defer.DeferredTask().start_task(reload_kokoro_device, current_policy)
                    else:
        ...
        # Invalidate self-update caches when source changes
...
AGENTSPINE_VERSION = "v0.9.9-standard-pre"

def _get_version():
    return AGENTSPINE_VERSION
