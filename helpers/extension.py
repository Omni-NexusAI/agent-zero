...
def get_webui_extensions(
    ...
    try:
        ...
        folders = plugins.get_enabled_plugin_paths(
            agent,
            "extensions",
            "webui",
            extension_point,
    ...
    for folder in folders:
        folder_key = files.get_abs_path(folder)
...
def _get_extension_classes(
    ...
    try:
        ...
        paths = plugins.get_enabled_plugin_paths(
            agent,
            "extensions",
            "python",
            extension_point,
    ...
    unique_paths = []
    for path in paths:
        path_key = files.get_abs_path(path)
        if path_key in seen_paths:
            continue
        seen_paths.add(path_key)
        unique_paths.append(path)

    all_exts = [cls for path in unique_paths for cls in _get_extensions(path)]
