...
const model = {
  ...
  formatJson() {
    try {
      ...
      this.setSettingsFieldConfigJson(formatted);
  ...
  setSettingsFieldConfigJson(val) {
  ...
  onClose() {
    ...
    this.setSettingsFieldConfigJson(val);
  ...
  async startStatusCheck() {
    ...
    while (this.statusCheck) {
      ...
      const interval = this.statusBurstCount > 0 ? 500 : 3000;
  ...
  async _applyConfig(configString, toggledServers = []) {
    ...
    try {
      ...
      if (normalizedConfig !== configString) {
        ...
        this.editor.clearSelection();
        this.setSettingsFieldConfigJson(normalizedConfig);
