...
const model = {
  ...
  applyFormState(source) {
    ...
    this.form.update_source =
      this.info?.active_update_source || "omni-nexusai";
  },
  ...
  async onUpdateSourceChanged() {
    try {
      let payloadSettings = null;
      if (settingsStore?.settings) {
        payloadSettings = JSON.parse(JSON.stringify(settingsStore.settings));
      } else {
        const current = await API.callJsonApi("settings_get", null);
        if (current?.settings) {
          payloadSettings = current.settings;
        }
      }
      if (!payloadSettings) {
        throw new Error("Unable to load settings for update source change.");
      }
      payloadSettings.self_update_source = this.form.update_source;
      ...
      if (response?.settings) {
        ...
        if (response.additional) {
          settingsStore.additional = response.additional;
        }
      ...
      this.error = e?.message || "Failed to save update source.";
    }
  },
