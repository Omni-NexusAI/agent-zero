...
def _config_helper():
    return _helper_from("config.py", "agentspine_convo_stt_config")


def _gpu_helper():
    return _helper_from("gpu.py", "agentspine_convo_stt_config_gpu")
...
def _save_runtime_config(helper, requested: dict) -> dict:
    ...
    stt["whisper"] = helper._deep_merge(stt.get("whisper", {}), normalized)
