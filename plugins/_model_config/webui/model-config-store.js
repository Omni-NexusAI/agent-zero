...
export const store = createStore("modelConfig", {
  ...
  // Provider model history for auto-restore on provider switch
  _providerModelHistory: {},
  _HISTORY_LS_KEY: 'modelConfig_providerHistory',
  ...
  init() {
    try {
      const raw = localStorage.getItem(this._HISTORY_LS_KEY);
      if (raw) this._providerModelHistory = JSON.parse(raw);
    } catch {}
  },
  ...
  // Provider change: auto-sync API base and restore last-used model
  onProviderChanged(sectionKey, model, oldProvider) {
    const newProvider = model.provider;
    if (!newProvider || newProvider === oldProvider) return;

    // Cache old provider's model before switching
    if (oldProvider && model.name?.trim()) {
      const cacheKey = `${sectionKey}_${oldProvider}`;
      this._providerModelHistory[cacheKey] = {
        name: model.name,
        api_base: model.api_base || '',
        ctx_length: model.ctx_length,
      };
      try {
        localStorage.setItem(this._HISTORY_LS_KEY, JSON.stringify(this._providerModelHistory));
      } catch {}
    }

    // Auto-sync API base for local providers
    const localDefaults = {
      lm_studio: 'http://host.docker.internal:1234/v1',
      ollama: 'http://host.docker.internal:11434',
    };
    const allLocalUrls = Object.values(localDefaults);

    if (newProvider in localDefaults) {
      if (!model.api_base?.trim() || allLocalUrls.includes(model.api_base)) {
        model.api_base = localDefaults[newProvider];
      }
    } else {
      if (allLocalUrls.includes(model.api_base)) {
        model.api_base = '';
      }
    }

    // Restore last-used model for the new provider
    const restoreKey = `${sectionKey}_${newProvider}`;
    const saved = this._providerModelHistory[restoreKey];
    if (saved?.name) {
      model.name = saved.name;
      if (saved.api_base !== undefined) model.api_base = saved.api_base || '';
      if (saved.ctx_length !== undefined) model.ctx_length = saved.ctx_length;
    } else {
      // Avoid stale model/provider pairings when this provider has no prior selection.
      model.name = '';
    }
  },
