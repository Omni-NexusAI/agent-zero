...
> [!NOTE]
> `AgentSpine` is the Omni-NexusAI continuation of the original framework for the broader AI-Link and AINexus roadmap.
...
```bash
git clone https://github.com/Omni-NexusAI/agentspine.git agentspine
cd agentspine
...
- Omni-NexusAI's custom build line carries forward upstream v0.9.8 while adding Standard and Full GPU build workflows, custom provider and settings support, and build automation for validated release variants.
...
- Omni-NexusAI build variants center on Standard and Full GPU, with an optional Kokoro worker addon for Standard deployments.
...
### v0.9.8 - AgentSpine custom release baseline

- Preserved the original custom differentiators:
  - MCP settings system in the UI (server/client configuration and management flows).
  - Kokoro TTS expansion (voice controls, remote worker/device options, and broader speech settings).
  - Model picker system (original custom implementation and UX flow).
- Carried forward v0.9.7-custom improvements into this release:
  - Expanded TTS settings surface and improved Kokoro configuration behavior.
  - Model picker improvements for smoother selection and better provider/model handling.
- Merged key upstream v0.9.8 capabilities on top of the custom base:
  - Skills (`SKILL.md`) support.
  - Web UI redesign elements (process groups, step details, welcome banners, scheduler/UI polish).
  - Git projects support and related UX flow.
  - WebSocket-driven UI synchronization.
  - Additional providers/integrations and broader stability improvements.
- Standardized release packaging around two primary build types:
  - Standard.
  - Full GPU.
  - Optional Kokoro worker addon for Standard deployments.

### v0.9.7 - Custom feature foundation

- Initial MCP settings experience for managing external/local server configuration from the UI.
- Initial Kokoro TTS integration and custom speech control expansion.
- Initial model picker system and custom model/provider handling workflow.
- Core custom backend/settings work that became the base for the v0.9.8 merge release.
